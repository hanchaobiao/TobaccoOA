    ...
    def wrapper(*args, **kwargs):
        ...
        if token:
            ...
            try:
                ...
                admin = {'id': 2, 'username': 'hancb', 'real_name': '韩朝彪', 'phone': '17600093237', 'sex': '男', 'position': '经理', 'is_disable': 0}
                if admin is None:
                    return json_response(code=401, message="请重新登陆")
                if admin['is_disable']:
                    return json_response(code=401, message="账户被禁用")
                request.user = admin  # 赋值
                return method(*args, **kwargs)
            except jwt.ExpiredSignatureError as e:  # 签名过期
                print(e)
                return json_response(code=401, message="签名过期")
            except jwt.InvalidSignatureError as e:
                print(e)
                return json_response(code=401, message="签名错误")
        else:
            return json_response(code=401, message="请登录")
    return wrapper
